...
public class Improved_Tracer : MonoBehaviour {
	...
	void OnCollisionEnter (Collision coll){
		...
		if (coll.gameObject.TryGetComponent(out singleplayertarget attack))
        {
            attack.TakeDamage(damage);
            Debug.Log("Hit!");
        }
				
		if (coll.gameObject.TryGetComponent(out enemyhealth enemyattack))
        {
            enemyattack.TakeDamage(damage);
            Debug.Log("Hit!");
        }

		if (coll.gameObject.TryGetComponent(out bosshealth bossattack))
        {
            bossattack.TakeDamage(damage);
            Debug.Log("Hit!");
        }
