...
public class PlayerMovement : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        slidecollider = GetComponent<CapsuleCollider>();
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.X) && Time.time >= nextTimetodash)
        {
            dashActive();
        ...
        //if (Input.GetKeyDown(KeyCode.B)) rb.velocity = new Vector3(rb.velocity.x, 40f, rb.velocity.z);
    ...
    private void dashActive()
    {
        ...
        if (grounded == true)
        {
            rig.AddForce(transform.forward * speedofdash, ForceMode.VelocityChange);
        }
        else
        {
            rig.AddForce(transform.forward * airdashspeed, ForceMode.VelocityChange);
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (camCon == null) 
        {
            Debug.Log($"camCon was null for {transform.root.name}", this.gameObject);
            return;
        }
        
        if (camCon == null) 
        {
        Debug.LogError($"camCon was null for {transform.root.name}", this.gameObject);
        Debug.Break();
        return;
        }
